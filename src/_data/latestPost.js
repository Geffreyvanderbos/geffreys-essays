...
export default async function () {
    ...
    try {
        ...
        let content = latest['content:encoded'] || latest.content || latest.description || "";

        const hasImage = /<img\b[^>]*>/i.test(content) ||
            (latest.enclosure && latest.enclosure.type && latest.enclosure.type.startsWith('image'));

        const hasVideo = /<video\b[^>]*>/i.test(content) ||
            (latest.enclosure && latest.enclosure.type && latest.enclosure.type.startsWith('video'));

        content = content.replace(/<img\b[^>]*>/gi, '');
        content = content.replace(/<video\b[^>]*>[\s\S]*?<\/video>/gi, '');

        content = content.replace(/<a\b[^>]*class="[^"]*hashtag[^"]*"[^>]*>([\s\S]*?)<\/a>/gi, (match, innerText) => {
            return innerText.replace(/<[^>]+>/g, '');
        });

        content = content.replace(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi, (match, attributes, innerText) => {
            const maxLen = 30;
