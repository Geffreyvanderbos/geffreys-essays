...
export default function (eleventyConfig) {
  ...
  eleventyConfig.addFilter("stripHtml", function (content) {
    ...
    // First, remove entire interactive/custom blocks with their content
    ...
    // Remove divs and other container elements that have classes (likely custom components)
    // This is a multi-pass approach to handle nested divs
    for (let i = 0; i < 5; i++) {
      html = html.replace(/<div\b[^>]*class[^>]*>[\s\S]*?<\/div>/gim, "");
    }

    // List of semantic tags we want to keep
    const allowedTags = ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li',
      'code', 'pre', 'blockquote', 'em', 'strong', 'b', 'i', 'a', 'br'];

    // Remove all attributes from opening tags, keep only tag name and href for links
    html = html.replace(/<(\w+)(\s+[^>]*)?>/gi, function (match, tag, attributes) {
      ...
      // For non-allowed tags, remove the opening tag
      return '';
    });

    // Remove closing tags for non-allowed elements
  ...
  eleventyConfig.addFilter("hasInteractive", function (content) {
    ...
    // Check for generic interactive elements (no hardcoded class names)
